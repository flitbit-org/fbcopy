...
namespace FlitBit.Copy
{
    ...
    {
        private static readonly MethodInfo GenericCreateType =
            typeof (CopierTypeFactory).MatchGenericMethod("ConcreteType",
                BindingFlags.Static | BindingFlags.NonPublic, 2, typeof (Type));

        private static readonly Lazy<EmittedModule> Module =
            new Lazy<EmittedModule>(() => RuntimeAssemblies.DynamicAssembly.DefineModule("Copiers", null),
                LazyThreadSafetyMode.ExecutionAndPublication
                );

        private static EmittedModule GeneratedModule
        {
        ...
        internal static Type ConcreteType(Type sourceType, Type targetType)
        {
            ...
            MethodInfo method = GenericCreateType.MakeGenericMethod(sourceType, targetType);
        ...
        internal static Type ConcreteType<TSource, TTarget>()
        {
            ...
            Type targetType = typeof (ICopier<TSource, TTarget>);
            string typeName = RuntimeAssemblies.PrepareTypeName(targetType, "Copier");

            lock (targetType.GetLockForType())
            {
                EmittedModule module = GeneratedModule;
                return module.Builder.GetType(typeName, false, false) ??
                       BuildCopierType<TSource, TTarget>(module, typeName);
            }
        }

        [SuppressMessage("Microsoft.Design", "CA1004:GenericMethodsShouldProvideTypeParameter",
            Justification = "By design.")]
        private static Type BuildCopierType<TSource, TTarget>(EmittedModule module, string typeName)
        {
            ...
            EmittedClass builder = module.DefineClass(typeName, EmittedClass.DefaultTypeAttributes,
                typeof (Copier<TSource, TTarget>), null);
        ...
        private static void ImplementPerformLooseCopy<TSource, TTarget>(EmittedClass builder)
        {
            ...
            method.ContributeInstructions((m, il) =>
            {
                foreach (
                    var prop in
                        from src in
                            typeof (TSource).GetReadablePropertiesFromHierarchy(BindingFlags.Instance |
                                                                                BindingFlags.Public)
                        join dest in
                            typeof (TTarget).GetWritablePropertiesFromHierarchy(BindingFlags.Instance |
                                                                                BindingFlags.Public)
        ...
        private static void ImplementPerformStrictCopy<TSource, TTarget>(EmittedClass builder)
        {
            MethodInfo baseMethod = typeof (Copier<TSource, TTarget>).GetMethod("PerformStrictCopy",
                BindingFlags.NonPublic | BindingFlags.Instance);
            EmittedMethod method = builder.DefineOverrideMethod(baseMethod);

            method.ContributeInstructions((m, il) =>
            {
                var props =
                    (from src in
                        typeof (TSource).GetReadablePropertiesFromHierarchy(BindingFlags.Instance | BindingFlags.Public)
                        select new
                        {
                            Source = src,
                            Destination =
                ...
                {
                    il.LoadValue(
                        "Cannot perfrom a strict copy because the source and target do not have all properties in common.");
